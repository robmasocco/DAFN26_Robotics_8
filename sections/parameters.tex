...
\begin{frame}{Node parameters}
  \justifying
	A ROS 2 node can have one or more \textbg{parameters}: values that can be specified at \textbg{startup}, changed at \textbg{runtime}, and used in the implementation.\\
  \bigskip
	The \textbg{parameter system} is \textbg{decentralized} and \textbg{built on messages and services}: each node has its own parameters and related services, and updates are \textbg{broadcasted} to every other node over the \texttt{/parameter\_events} topic (try to inspect it! May want to log it too during experiments!).\\
  \bigskip
	Parameters can be \textbg{listed}, \textbg{queried}, \textbg{described} and \textbg{set}, using either \textbg{CLI tools} or \textbg{service calls}; \textbg{YAML} configuration files may be \textbg{loaded} or \textbg{dumped}.\\
  \bigskip
	It is possible to specify what to do when a parameter update is requested by defining a \textbg{callback}.\\
  \bigskip
...
\begin{frame}{Parameters CLI commands}
  \justifying
	\begin{itemize}
		...
		\item \texttt{ros2 param dump NODE\_NAME}\\Dumps the current parameter configuration on \texttt{stdout}.
...
\begin{frame}{Parameters services}
  ...
  \end{itemize}
  Try to inspect them with \texttt{ros2 service type}, or call them from the command line! They all take \textbg{groups of parameters} on which to operate, and can be called either from a \textbg{CLI tool} or from a \textbg{client node}: that is the way to go if you want to \textbg{change parameters from software}.
\end{frame}
...
\begin{frame}{Coding with parameters}{Suggested TODO list}
  \begin{enumerate}
    \item Define a \textbg{callback} to be called when a parameter is updated (there is an API for this).
...
\begin{frame}{Coding with parameters}{About declarations}
  ...
  \begin{enumerate}
    ...
    \item the \textbg{complete way}: \texttt{declare\_parameter(...)} specifying \textbg{all the information} about the parameter, including its \textbg{type}, \textbg{description}, \textbg{read-only} flag, \textbg{max} and \textbg{min} values, \textbg{constraints} and more, using an \texttt{rcl\_interfaces::msg::ParameterDescriptor} object.
  \end{enumerate}
  \begin{alertblock}{}
    \centering
