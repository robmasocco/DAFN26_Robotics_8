...
\begin{frame}{Composable nodes}{Isolation vs integration}
  ...
  \bigskip
  On the other hand, if all nodes run \textbg{in the same process}, they can \textbg{share resources and memory} and ease the burden on the OS, but a \textbg{critical error} in one node may \textbg{affect the others}.\\
  \bigskip
...
\begin{frame}{Composable nodes}{The building blocks of a control architecture}
  ...
  \bigskip
  Inside the container, the \texttt{main()} executor schedules \textbg{all jobs}, and nodes use \textbg{shared memory} to communicate, \textbg{bypassing the underlying RMW implementation}.\\
  \bigskip
  Nodes can be \textbg{loaded} (constructed) and \textbg{unloaded} (destructed) at runtime, without restarting the container.\\
  \bigskip
  The container process contains a \texttt{rclcpp\_components::ComponentContainer} \textbg{node}, that exposes the \texttt{\textasciitilde/load\_node}, \texttt{\textasciitilde/list\_nodes}, and \texttt{\textasciitilde/unload\_node} \textbg{services}.
\end{frame}
...
\begin{frame}{Composable nodes}{Best practices}
  ...
  \begin{enumerate}
    ...
    \item add \textbg{macros} to your node class definition code to register the component class (usually at the end of one node source file, mainly the one where the constructor is);
